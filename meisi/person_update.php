<?php
    ...
    if($image != "error"){
        ...
        $pdo = pdo_init();
        $sql = "UPDATE person 
        SET person_name = :person_name,person_division = :person_division,person_phone_number = :person_phone_number,person_mail_address = :person_mail_address,person_updated_user = :person_updated_user,person_updated_date = :person_updated_date 
        ...
        $stmt->bindValue(':person_updated_user', $_POST["person_updated_user"], PDO::PARAM_STR);
        $stmt->bindValue(':person_updated_date', date('Y-m-d'), PDO::PARAM_STR);
        $status = $stmt->execute();
        if ($status == false) {
            sqlError($stmt);
        }
        ...
        header("Location: person.php?person_id=". $_GET['person_id']);
        exit;
    }
?>
